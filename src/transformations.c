    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            ...
            if(new_j < 0 ||new_j >= cols ||new_i < 0 ||new_i >= rows) {
                rotatedImage[i][j].r = FILL;
                rotatedImage[i][j].g = FILL;
                rotatedImage[i][j].b = FILL;
            } else {
                rotatedImage[i][j].r = image[new_i][new_j].r;
                rotatedImage[i][j].g = image[new_i][new_j].g;
                rotatedImage[i][j].b = image[new_i][new_j].b;
            }
        }
    }
}
